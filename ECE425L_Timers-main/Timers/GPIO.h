...
/**
 * @brief the Change_Counter_Speed function adjusts the counter's toggle speed based on the button status.
 *
 * This function takes the status of the buttons as input and cahnges the counter's toggle rate. The function
 * checks which button is pressed and sets the toggle rate accordingly. It returns the updated toggle rate
 * in ms.
 *
 * Button status is determined as follows:
 *  - 0x00: No buttons pressed, toggle rate is set to 500 ms
 *  - 0x01: SW5 is pressed, toggle rate is set to 50 ms
 *  - 0x02: SW4 is pressed, toggle rate is set to 100 ms
 *  - 0x04: SW3 is pressed, toggle rate is set to 200 ms
 *  - 0x08: SW2 is pressed, toggle rate is set to 1000 ms
 *
 * If no button is pressed or the status doesn't match any case, the default toggle rate is 500 ms.
 *
 * @param button_status an integer representing the current button status from SW2 to SW5.
 *
 * @return The toggle rate in milliseconds based on the chosen button.
 */
