...
{
    // default toggle rate to 500 ms
    uint32_t toggle_rate_ms = 500;

    // check the button status using a switch-case
    switch(button_status)
    {   
        // no buttons are pressed / default toggle rate (500 ms)
        case 0x00:
        {
            toggle_rate_ms = 500; // initial debugging consisted of accidentally leaving in a preiod EduBase_LED_Output which caused the LEDs to malfunction
																	// and remain on as well as turning on the wrong leds when the buttons were pushed. 
																	// Had to remove the EduBase_LEDs_Output(RGB_LED_OFF) as it would also cause the LEDS to malfunction
        }
        break;      
        // SW5 is pressed, set toggle rate to 50 ms
        case 0x01:
        {
            toggle_rate_ms = 50;
        }
        break;        
        // SW4 is pressed, set toggle rate to 100 ms
        case 0x02:
        {
            toggle_rate_ms = 100;
        }
        break;        
        // SW3 is pressed, set toggle rate to 200 ms
        case 0x04:
        {
            toggle_rate_ms = 200;
        }
        break;       
        // SW2 is pressed, set toggle rate to 1000 ms
        case 0x08:
        {
            toggle_rate_ms = 1000;
        }
        break;          
        // default case, keep toggle rate at 500 ms
        default: 					
            toggle_rate_ms = 500;				
        break;
    }
    
    // return the final toggle rate to be updated
